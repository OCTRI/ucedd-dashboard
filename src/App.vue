...
const categories = computed(() => {
  const categorySet = new Set<string>(
    csvData.value.map(row => row.stratification_category)
  );
...
const fetchMeasureDescription = async () => {
  ...
  if (apiKey) {
    response = await fetch(
      'https://docs.google.com/spreadsheets/d/1_jFImMo4fZd8vQ7x4wAyLzxnnkn95F-vSXCV-yLzbi4/export?gid=0&format=csv&id=1_jFImMo4fZd8vQ7x4wAyLzxnnkn95F-vSXCV-yLzbi4&key=' +
        apiKey
    );
  } else {
...
const initialize = (results: Papa.ParseResult<DataRow>) => {
  ...
  csvData.value = results.data.map(obj => ({
    ...obj,
  ...
  <div class="tab-content" id="dashboardTabContent">
    ...
    >
      <BarChart
        ...
        :measureInfo="{ ...selectedMeasureRow }"
